...
double runExDOT(const double *d_x, const double *d_y, long long int* d_PartialSuperaccs, int N, size_t size){
    /*
...
double cuda_ExBLAS_dot_product(const Vector *x, const Vector *y, long long int* d_PartialSuperaccs, size_t size){
    ...
    */
    ...
    double dot = runExDOT(x->d_data, y->d_data, d_PartialSuperaccs, x->size, size);
    return dot;
