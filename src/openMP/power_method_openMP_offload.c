#include "power_method_openMP_offload.h"
#include <stdio.h>
#include <stdbool.h>
 ...
 */
double openMP_offload_dense_power_method(denseMatrix* A){
    ...
    do{
        ...
        openMP_offload_dense_matvec_mult(A, x);
        openMP_offload_normalize_vector(x);
        lambda_new = openMP_offload_dense_approximate_eigenvalue(A, x);
    } while(openMP_offload_convergence(lambda_new, lambda_old, 0.00001));
 ...
 */
bool openMP_offload_convergence(double lambda_new, double lambda_old, double threshold){
...
void openMP_offload_dense_matvec_mult(denseMatrix* A, Vector* x){
 ...
 * @return Nothing. The result is stored directly in the vector x.
 */
void openMP_offload_normalize_vector(Vector* x){

    double norm = sqrt(openMP_offload_dot_product(x, x));
    if (norm == 0) return;
 ...
 */
double openMP_offload_dense_approximate_eigenvalue(denseMatrix* A, Vector* x){
    Vector copy;
    copy.size = x->size;
    ...
    openMP_offload_dense_matvec_mult(A, &copy);
    double lambda = openMP_offload_dot_product(x, &copy);
...
double openMP_offload_dot_product2(Vector* x, Vector* y){
    if(x->size != y->size){
...
double openMP_offload_dot_product(Vector* x, Vector* y){
    if(x->size != y->size){
        printf("Error: Vectors must have the same size (x: %d, y: %d)\n", x->size, y->size);
        return 0.0;
    }
 ...
 */
 ...
 double openMP_offload_sparse_power_method(SparseMatrixAny* A){
 ...
 // initial vector
    ...
    do{
        lambda_old = lambda_new;
        openMP_offload_sparse_matvec_mult(A, x);
        openMP_offload_normalize_vector(x);
        lambda_new = openMP_offload_sparse_approximate_eigenvalue(A, x);
    } while(openMP_offload_convergence(lambda_new, lambda_old, 0.00001));
 ...
 */
 ...
 void openMP_offload_sparse_matvec_mult_CSR(sparseMatrixCSR* A, Vector* x){
...
void openMP_offload_sparse_matvec_mult(SparseMatrixAny* A, Vector* x){
    
    if (A->type == CSR) {
        openMP_offload_sparse_matvec_mult_CSR(A->mat.csr, x);
    } else {
        printf("Runtime error: OpenMP currently only works with CSR format\n");
        exit(EXIT_FAILURE);
    }
...
double openMP_offload_sparse_approximate_eigenvalue(SparseMatrixAny* A, Vector* x){
    Vector copy;
    copy.size = x->size;
    copy.data = malloc(sizeof(double) * copy.size);
    for(int i = 0; i < copy.size; i++){
    ...
    openMP_offload_sparse_matvec_mult(A, &copy);
    double lambda = openMP_offload_dot_product(x, &copy);
